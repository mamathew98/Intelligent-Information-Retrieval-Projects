...
public class MyAdditiveScoringIterator extends ScoringFunctionIterator implements DeltaScoringIterator {
...
//   private final double k;
  ...
  public MyAdditiveScoringIterator(NodeParameters np, LengthsIterator ls, CountIterator it)
          throws IOException {
    super(np, ls, it);

    // statistics for BM25 scoring
    delta = np.get("delta", 0.75);
    // k = np.get("k", 1.2);
  ...
  private double score(double count, double length) {
    ...
    double denominator = length + (vocab * delta);
