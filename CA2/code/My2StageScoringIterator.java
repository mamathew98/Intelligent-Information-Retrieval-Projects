...
public class My2StageScoringIterator extends ScoringFunctionIterator
  ...
  public My2StageScoringIterator(NodeParameters p, LengthsIterator ls, CountIterator it)
          throws IOException {
    super(p, ls, it);

    // stats
    mu = p.get("mu", 1500D);
    lambda = p.get("lambda", 0.5);
  ...
  private double dirichletScore(double count, double length) {
    ...
    double dir = numerator / denominator;
    double twoStage = ((1 - lambda) * dir) + (lambda * background);
    return Math.log(twoStage);
  }
}
