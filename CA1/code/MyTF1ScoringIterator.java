...
public class MyTF1ScoringIterator extends ScoringFunctionIterator implements DeltaScoringIterator {
  ...
  public MyTF1ScoringIterator(NodeParameters np, LengthsIterator ls, CountIterator it)
          throws IOException {
    super(np, ls, it);
  ...
  private double score(double count, double length) {
    ...
    long df = np.getLong("nodeDocumentCount");
    return 1 * ((Math.log(1 + Math.log(1 + count))) / denominator) * ((documentCount + 1.0) / df);
  }
