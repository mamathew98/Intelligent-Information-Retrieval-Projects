...
public class MyTF2ScoringIterator extends ScoringFunctionIterator implements DeltaScoringIterator {
  ...
  public MyTF2ScoringIterator(NodeParameters np, LengthsIterator ls, CountIterator it)
          throws IOException {
    super(np, ls, it);
  ...
  private double score(double count, double length) {
    ...
    long df = np.getLong("nodeDocumentCount");
    return 1 * ((1 + Math.log(1 + count)) / denominator) * ((documentCount + 1.0) / df);
  }
